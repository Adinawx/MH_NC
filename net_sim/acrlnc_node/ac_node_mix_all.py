...
class ACRLNC_Node_Mix_All():
    ...
    def update_pt_buffer_add_all(self, in_packet_info, in_packet_recep_flag):
        ...
        if in_packet_recep_flag:
            ...
            if self.node_index == 1 or 'NEW' in self.in_pt.fec_type or (
                    'NEW' not in self.in_pt.fec_type and self.accept_fec()):
                ...
                if isinstance(self.in_pt.nc_header, list):
                    if in_packet_info.nc_header[1][1] is not None:
                        curr_info_wmax = self.in_pt.nc_header[1][1]

                        if curr_info_wmax > self.last_dec_id_info:
                            self.rece_buffer.put(self.in_pt)
                else:
                    if in_packet_info.nc_header is not None:
                        curr_info_wmax = self.in_pt.nc_header
    ...
    def decode_and_create_fb(self, in_packet_recep_flag, fb_packet):
        ...
        if fb_packet is not None:  # Feedback packet is not empty.
            ...
            if dec_id > self.last_dec_id:  # decoding.
                ...
                ### WE dont look at semi-dec for the mix version, It is probably calculated wrong.###
    ...
    def update_pt_buffer_discard(self, fb_packet):
        ...
        if self.in_cur_fb[1] is not None:  # Some feedback is arrived.
            ...
            if self.node_type == 'Transmitter':
                ...
                if dec_id >= 0 and self.out_cur_fb[
                    2] >= -1:  # Indicator of decoding in the current node, meaning it is indeed OK to discard packets.
    ...
    def update_packet_info_num(self):
        ...
        if len(self.rece_buffer) > 0:  # Buffer is not empty.
            ...
            last_packet_wmax = last_packet.nc_header[1][1] if isinstance(last_packet.nc_header,
                                                                         list) else last_packet.nc_header
    ...
    def output_packet_processing(self, in_packet_recep_flag, fb_packet, in_packet_info):
        ...
        if self.node_type == 'Transmitter':  # For MIXALL, do it only in the encoder
            if self.in_cur_fb[2] is not None and self.in_cur_fb[2] >= 0:
                self.fb_dec_id = self.in_cur_fb[2]
            if self.fb_dec_id is not None:  # If some decoding was done.
                # Set a specific capacity for the FIFO_Store
                capacity = len(self.pt_buffer)
                if capacity == 0:
                    capacity = float('inf')
                pt_buffer_cut = FIFO_Store(self.env, capacity=capacity, memory_size=capacity, debug=False)
