...
class NC_enc:
    ...
    def run(self):
        ...
        while True:
            ...
            ### MY CHANGE 27/5 #####################
            mem_items = self.store.items
            nc_headers = [item.nc_header for item in mem_items]
            if len(nc_headers) > 0:
                print(f"In Enc: list is {mem_items}")
                self.out.nc_header = [nc_headers[0], nc_headers[-1]]  # MY CHANGE 27/5
            else:
                print(f"In Enc: list is {mem_items}")
            ### END MY CHANGE 27/5 #################
    ...
    def put_mem(self, mem_packet):
        """Sends a packet to this element."""
        return self.store.put(mem_packet)
