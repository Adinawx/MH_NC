...
def print_for_paper(cfg, dfs, labels, er_rates, node_value=None, filename=None):
    ...
    if filename:
        ...
        plt.savefig(os.path.join(filename, f"Node_{n}_Data_Rates_vs_eps.png"), dpi=300)
    ######################################################################################################################
    ...
    for metric in metrics_to_plot:
        ...
        for idx, n in enumerate(node_value):
            ...
            if 'Mean' in metric:
                if n != -1:
                    srArq_filename = os.path.join(srArq_path, f"node_{n + 1}", "srARQ_dmean.txt")
                else:
                    srArq_filename = os.path.join(srArq_path, f"node_{N}", "srARQ_dmean.txt")

            elif 'Max' in metric:
                if n != -1:
                    srArq_filename = os.path.join(srArq_path, f"node_{n + 1}", "srARQ_dmax.txt")
                else:
                    srArq_filename = os.path.join(srArq_path, f"node_{N}", "srARQ_dmax.txt")
        ...
        if filename:
            ...
            plt.savefig(os.path.join(filename, f"{metric}_All_vs_eps.png"), dpi=300)
        ######################################################################################################################
...
def print_for_arxiv(cfg, dfs, labels, er_rates, node_value=None, filename=None):
    ...
    for curr_n in range(1, max_n + 1):
        ...
        if filename:
            ...
            plt.savefig(os.path.join(filename, f"Node_{curr_n}_Combined_Metrics_vs_eps.png"), dpi=300)
