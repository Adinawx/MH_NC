...
def create_network_and_run(cfg, rtt, er_rates):
    ...
    for curr_ch in range(num_of_nodes - 1):
        ...
        if er_load == 'erasure':
            ...
            ff_channels.append(env, delay_dist=rtt / 2, noise_dict=set_sim_params.noise_param(er_load, path), wire_id=curr_ch, debug=False)
...
def run_1(cfg, rtt, er_rates, new_folder):
    ...
    with open(os.path.join(new_folder, f"metrics.txt"), "a") as f:
        ...
        f.write(f"last dec: {last_dec}, bls_empty: {run_log[0].empty_bls_num}, buffer_empty: {run_log[0].empty_buffer_num}\n")
