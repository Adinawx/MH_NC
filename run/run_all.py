...
def print_metrics_for_all_dfs(dfs, labels, er_rates, node_value=None, filename=None, type_mid='Real'):
    ...
    for n in node_value:  # Plot for each node           
        ...
        if n == -1:
            ...
            if type_mid == 'Real':
                metrics_to_plot = ['Normalized Goodput Real', 'Delivery Rate Real', 'Channel Usage Rate Real', 'Mean Real Delay', 'Max Real Delay']
        ...
        fig, axes = plt.subplots(1, len(metrics_to_plot), figsize=(5 * len(metrics_to_plot), 6), sharex=True, sharey=False)
        ...
        for n in node_value:  # Plot for each node
            ...
            for column in metrics_to_plot:
                ...
                for df_node, label in zip(dfs_mean_node, labels):
                    ...
                    plt.plot(df_node['Eps'], df_node[column], label=f'{plt_label}', linestyle='-', alpha=0.7, linewidth=2, color=color)
...
def load_and_plot(dfs_names, node_value=None):
    ...
    plot_results.print_for_paper(cfg=cfg, dfs=df_list, labels=dfs_names, er_rates=er_rates, filename=filename, node_value=node_value)
    plot_results.print_for_arxiv(cfg=cfg, dfs=df_list, labels=dfs_names, er_rates=er_rates, filename=filename, node_value=node_value)


def main_run():
    ##########################################################################################################
    # Step 1: Run simulation    
    # Step 2: Save Plot Options:    
    #   1: Right after run.
    #   2: Load results and plot (Saves figures into the results folder).
    ##########################################################################################################
    ...
    # Step 2: Save Plot option 1 - metrics for MIXALL
    cfg = Config.from_json(CFG)
    filename = os.path.join(cfg.param.results_folder, cfg.param.results_filename_base)
    print_metrics_for_all_dfs(dfs=[df_mixall],
                              labels=['MIXALL'],
                              er_rates=cfg.param.er_rates,
                              filename=filename)

    ######  Load and Plot  ######
    # Step 2: Save Plot option 2 - Run after simulatio is done. Takes results by the config.param.results_filename_base and dfs_name.
    # dfs_names = ['MIXALL']
    ...
    ######  Run All nodes AC-RLNC  ######
    # # Step 1: Run simulation - All nodes AC-RLNC:
    # df_ac_fec = run_all(prot_type="AC-FEC")
    # # Step 2: Save Plot option 1 - all results:
    # cfg = Config.from_json(CFG)
    # filename = os.path.join(cfg.param.results_folder, cfg.param.results_filename_base)
    # print_metrics_for_all_dfs(dfs=[df_ac_fec,df_mixall],
    #                           labels=['AC-FEC',"MIXALL"],
    #                           er_rates=cfg.param.er_rates,
    #                           filename=filename)
    ...
    # cfg = Config.from_json(CFG)
    # filename = os.path.join(cfg.param.results_folder, cfg.param.results_filename_base)
    # print_metrics_for_all_dfs(dfs=[df_ac_fec, df_bs_fec, df_ac_empty, df_bs_empty, df_mixall],
    #                           labels=['AC-FEC', 'BS-FEC', 'AC-EMPTY', 'BS-EMPTY', 'MIXALL'],
    #                           er_rates=cfg.param.er_rates,
    #                           filename=filename)
